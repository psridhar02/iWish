...
export default function Product(){
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.msg || err.message || 'Error loading product');
    } finally {
      setLoading(false);
    }
  };
  ...
  const save = async () => {
    // 2. REPLACE alert('Login to save') with show(...)
    if (!getToken()) return show('Login to save', 'warning'); // Using 'warning' type for non-critical alerts
  ...
  if (error) return (
    <div className="container mx-auto px-4 py-8">
      <div className="bg-red-50 border border-red-100 p-4 rounded">
        ...
        <button className="bg-iwish-500 text-white px-3 py-1 rounded" onClick={fetchProduct}>Retry</button>
      </div>
    </div>
  );

  if (!prod) return <div className="container mx-auto px-4 py-8">Product not found</div>;

  return (
    <div className="md:flex gap-8 container mx-auto px-4 py-8">
      <img src={prod.image} className="w-full md:w-1/3 h-96 object-contain" alt={prod.title} />
      <div className="md:flex-1">
        ...
        <button onClick={save} className="mt-6 bg-iwish-500 text-white px-4 py-2 rounded">Save to Wishlist</button>
      </div>
    </div>
  );
}